    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="card p-5 mb-8 animate-fade-in-up">
        <div className="flex flex-col sm:flex-row sm:items-end gap-4">
          ...
          {cartItems.length > 0 && (
            <button onClick={() => router.push("/rentals/manage")} className="btn-secondary text-sm shrink-0">
              View Cart ({cartItems.length})
            </button>
          )}
        </div>
        {pickupDate && !returnDate && (
          <p className="text-xs text-amber-600 mt-2">Please select a return date to see availability</p>
        )}
      </div>
                  ...
                  {unavailable && (
                    <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                      <span className="text-white text-sm font-semibold bg-red-600 px-3 py-1 rounded-full">
                        {!car.available ? "Unavailable" : booked ? "Booked" : "In Cart"}
                      </span>
                    </div>
                  )}
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {!unavailable && datesSelected && (
                      <button onClick={() => handleAddToCart(car)} className="btn-primary text-xs px-4 py-2">Add to Cart</button>
                    )}
                    {!unavailable && !datesSelected && (
                      <span className="text-xs text-slate-400">Select dates first</span>
                    )}
                  </div>
                </div>
              </div>
