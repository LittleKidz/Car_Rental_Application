...
export default function ManageRentalsPage() {
  ...
  const totalCost = cartItems.reduce((sum, item) => {
    return (
      sum + item.car.dailyRate * calcDays(item.rentalDate, item.returnDate)
    );
  }, 0);
    ...
    for (const item of cartItems) {
      try {
        ...
        if (data.success) {
          res.push({
            ok: true,
            msg: `${item.car.brand} ${item.car.model} — Booked!`,
          });
        } else {
          res.push({
            ok: false,
            msg: `${item.car.brand} ${item.car.model} — ${data.message || "Failed"}`,
          });
        }
      } catch {
        res.push({
          ok: false,
          msg: `${item.car.brand} ${item.car.model} — Error`,
        });
      }
    }
    ...
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12">
      ...
      <div className="mb-10 animate-fade-in-up">
        ...
        <p className="text-slate-500">
          Review your selections and confirm bookings
        </p>
      </div>
                  ...
                  <div className="flex items-center gap-0">
                    ...
                    <div className="flex-1 min-w-0 px-4 py-3">
                      ...
                      <p className="text-sm text-slate-500">
                        {item.provider.name} · {item.car.licensePlate}
                      </p>
                      ...
                      <p className="text-sm font-bold text-slate-700 mt-1">
                        ฿{(item.car.dailyRate * days).toLocaleString()}
                      </p>
                    </div>
                    <button
